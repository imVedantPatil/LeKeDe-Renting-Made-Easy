...
public class Product_category_sports extends AppCompatActivity
{
    private Spinner category,duration;
    private EditText rent,deposit,timespan,description;
    public String rentText,depositText,timespanText,descriptionText,dayText,categoryText,uri,name,email,phone_number;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_product_sports);

        category=findViewById(R.id.select_category);
        ArrayAdapter<CharSequence> category_adaptor=ArrayAdapter.createFromResource(this,R.array.sub_category_sport,android.R.layout.simple_spinner_item);
        ...
        childref.child(user.getUid()).addValueEventListener(new ValueEventListener()
        {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                try {
                    ...
                    Toast.makeText(Product_category_sports.this, "User Data Successfully Fetched", Toast.LENGTH_SHORT).show();
                }
                catch (Exception e)
                {
                    Toast.makeText(Product_category_sports.this, ""+e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
            ...
            {
                Toast.makeText(Product_category_sports.this, "Error!!!!1,Data Unccessfully Fetched"+databaseError.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    private void getData()
    {
        ...
        if((!checkEmptyCategory(categoryText))  && (!checkEmptyRent(rentText)) && (!checkEmptyDeposit(depositText)) && (descriptionText.isEmpty()) && (!checkEmptyTimespan(timespanText,dayText)))
        {
            ...
            childref.child("Sports").child(user.getUid()).setValue(product);
            finish();
            startActivity(new Intent(Product_category_sports.this,Addpost_finish.class));
